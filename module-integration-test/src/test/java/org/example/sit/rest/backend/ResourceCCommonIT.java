...
public class ResourceCCommonIT extends RestResourceTestFactory {
   ...
   public void test_HttpsWithoutBasicAuth() {
      ...
      RestAssured.given().spec(requestSpecification).when().auth().none()
            .get(PATH_REST_BACKEND_RESOURCE_C).then().statusCode(HttpStatus.SC_UNAUTHORIZED);
   }
   ...
   public void test_HttpsWithBasicAuthInvalidRole() {
      ...
      RestAssured.given().spec(requestSpecification).when().auth()
            ...
            .get(PATH_REST_BACKEND_RESOURCE_C).then().statusCode(HttpStatus.SC_FORBIDDEN);
   }
   ...
   public void test_UnsupportedContentType() {
      ...
      RestAssured.given().spec(requestSpecification).when()
            .get(PATH_REST_BACKEND_RESOURCE_C).then().statusCode(HttpStatus.SC_NOT_ACCEPTABLE);
   }
}
